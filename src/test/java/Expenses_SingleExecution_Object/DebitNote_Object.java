...
public class DebitNote_Object extends BaseClass{
	...
	public DebitNote_Object(WebDriver BrowserDriver){
	...
	public void CreateDebitNote() throws Throwable {
		...
		for(int i=1;i<=DebitNote_Rows;i++) {
            ...
            for(int k=1;k<=allWo.size();k++) {
            	if(getWorkOrderDD().isDisplayed() && getWorkOrderDD().isEnabled() && allWo.size()>0) {
            		AppInd.selectByIndex(getWorkOrderDD(), 1);
            		break;
            	}
            }
            Thread.sleep(2000);
            ...
            System.out.println("3. Expenses Debit Note ID : "+DbtNoteID+" Processed with: "+DebitNoteAmount+" INR");
            Thread.sleep(2000);
