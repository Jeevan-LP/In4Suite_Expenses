...
public class Payment_Object extends BaseClass{
	...
	public Advance_Object AdvObj;
	public DebitNote_Object DbtNoteObj;
	
	
	public Payment_Object(WebDriver BrowserDriver,WorkOrder_Object WoObject, Advance_Object AdvObj, DebitNote_Object DbtNoteObj){
	...
	public void CreatePayment() throws Throwable {
		...
		for(int i=1;i<=Payment_Rows;i++) {
			...
			/////Advance Details
			List<WebElement> AdvIDS = BrowserDriver.findElements(By.xpath
			("//table[@class='in4-table']//a[contains(@id, 'dgAdavance_ctl')]"));
			
			List<WebElement> AdvAllTextF = BrowserDriver.findElements(By.xpath
			("//table[@class='in4-table']//input[contains(@onchange, 'getAdvanceOutstanding();CalculateVATAmountAdditionTax')]"));
			
			for(int k=0;k<AdvIDS.size();k++) {
				if(AdvObj.AdvanceID.equals(AdvIDS.get(k).getText())) {
					WebElement AdvanceDeducted = AdvAllTextF.get(k);
					Thread.sleep(2000);
					AdvanceDeducted.clear();
					Thread.sleep(2000);
					AppInd.selectTextFieldFromExcel(data.XLFilepath, data.XlShee7, i, 9, AdvanceDeducted);					Thread.sleep(2000);
					break;
				}
			}
			Thread.sleep(2000);
			...
			/////Debit Note Details
			List<WebElement> DbtIDS = BrowserDriver.findElements(By.xpath
			("//table[@class='in4-table']//a[contains(@id, 'gvDebitNote_ctl')]"));
					
			List<WebElement> DbtAllTextF = BrowserDriver.findElements(By.xpath
			("//table[@class='in4-table']//input[@onblur='return CalculateDebitNoteDetails();']"));
					
			for(int l=0;l<DbtIDS.size();l++) {
				if(DbtNoteObj.DbtNoteID.equals(DbtIDS.get(l).getText())) {
					WebElement DebitNoteDeducted = DbtAllTextF.get(l);
					Thread.sleep(1500);
					DebitNoteDeducted.clear();
					Thread.sleep(1500);
					AppInd.selectTextFieldFromExcel(data.XLFilepath, data.XlShee7, i, 10, DebitNoteDeducted);
					break;
				}
			}
			Thread.sleep(2000);
            ...
            System.out.println("5. Expenses Payment Request Id: "+PaymentID+" Paid amount is: "+paidAmount+" INR"+" for "+WoObject.WODisplayNo);
            List<WebElement> AdvanceDeductedAmount = BrowserDriver.findElements(By.xpath("//table[@class='in4-table']//td[@align='right']//span[contains(@id, 'dgAdavance_ctl')]"));
            if (!AdvanceDeductedAmount.isEmpty()) {
			    // Get text from the last element
			    String lastText1 = AdvanceDeductedAmount.get(AdvanceDeductedAmount.size() - 1).getText();
			    
			    // Print it
	            System.out.println(" a. Advance "+AdvObj.AdvanceID+" Adjusted amount is: "+lastText1);
			} else {
			    System.out.println("No Advance amounts found.");
			}
			List<WebElement> DebitNoteDeductedAmount = BrowserDriver.findElements(By.xpath("//table[@class='in4-table']//td[@align='right']//span[contains(@id, 'gvDebitNote_ctl')]"));
			if (!DebitNoteDeductedAmount.isEmpty()) {
			    // Get text from the last element
			    String lastText2 = DebitNoteDeductedAmount.get(DebitNoteDeductedAmount.size() - 1).getText();
			    
			    // Print it
	            System.out.println(" b. Debit Note "+DbtNoteObj.DbtNoteID +" Adjusted amount is: "+lastText2);
			} else {
			    System.out.println("No debit note amounts found.");
			}
